	...
	class GeometryParser {
		...
		genGeometry( geoNode, skeleton, morphTargets, preTransform ) {
			...
			if ( buffers.colors.length > 0 ) {
				...
				// --- 修复：从 buffers.alphas 读取数据，而不是 geoInfo ---
				if ( buffers.alphas && buffers.alphas.length > 0 ) {
					geo.setAttribute( 'alphaColor', new THREE.Float32BufferAttribute( buffers.alphas, 3 ) );
				}
				// ---------------------------------------------------
			}
			if ( skeleton ) {
		...
		genBuffers( geoInfo ) {
			...
			let faceNormals = [];
			let faceColors = [];
			let faceAlphas = []; // --- 新增：当前面的 Alpha 数据 ---
			let faceUVs = [];
			let faceWeights = [];
			...
			geoInfo.vertexIndices.forEach( function ( vertexIndex, polygonVertexIndex ) {
				...
				if ( geoInfo.color ) {
					...
					if ( data.length === 4 ) {
						const a = data[ 3 ];
						faceAlphas.push( a, a, a ); 
					} else {
						faceAlphas.push( 1, 1, 1 ); 
					}
				}
				if ( geoInfo.skeleton ) {
				...
				if ( endOfFace ) {
					// --- 修复：传入 faceAlphas ---
					scope.genFace( buffers, geoInfo, facePositionIndexes, materialIndex, faceNormals, faceColors, faceAlphas, faceUVs, faceWeights, faceWeightIndices, faceLength );
		...
		genFace( buffers, geoInfo, facePositionIndexes, materialIndex, faceNormals, faceColors, faceAlphas, faceUVs, faceWeights, faceWeightIndices, faceLength ) {
			for ( let i = 2; i < faceLength; i ++ ) {
				...
				if ( geoInfo.color ) {
					...
					// --- 修复：同样处理 Alpha ---
					buffers.alphas.push( faceAlphas[ 0 ] );
					buffers.alphas.push( faceAlphas[ 1 ] );
					buffers.alphas.push( faceAlphas[ 2 ] );
					buffers.alphas.push( faceAlphas[ ( i - 1 ) * 3 ] );
					buffers.alphas.push( faceAlphas[ ( i - 1 ) * 3 + 1 ] );
					buffers.alphas.push( faceAlphas[ ( i - 1 ) * 3 + 2 ] );
					buffers.alphas.push( faceAlphas[ i * 3 ] );
					buffers.alphas.push( faceAlphas[ i * 3 + 1 ] );
					buffers.alphas.push( faceAlphas[ i * 3 + 2 ] );
				}
				if ( geoInfo.material && geoInfo.material.mappingType !== 'AllSame' ) {
		...
		parseVertexColors( ColorNode ) {
			...
			// --- 修正后的逻辑：优先判断是否为 4 通道 (RGBA) ---
			// FBX 标准通常是 RGBA。如果长度是 4 的倍数，我们优先当作 4 通道处理
			// 除非它同时也能被 3 整除且不能被 4 整除（这种情况极少见，通常作为 3 通道）
			let dataSize = 3;
